...
def execute_query():
    ...
    for course in CourseMetaData.objects.select_related('canonical_course_run', 'level_type', 'video').all():
...
def create_or_update_course(meta_data_course, available_organization):
    ...
    is_publishable = (
            meta_data_course.partner.has_marketing_site and
    ...
    if is_publishable:
        publisher = CourseRunMarketingSiteWordpressPublisher(meta_data_course.partner)

        publisher.publish_obj(meta_data_course)
